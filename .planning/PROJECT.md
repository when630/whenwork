...
- [ ] 앱 이름은 WHENWORK를 유지한다 (2026-09-15 개명 계획 철회 — WHENNOTE는 다른 프로젝트 이름). 패키지명·productName·appId·설치 파일명·트레이/창 제목·데이터 폴더·문서가 일관되게 WHENWORK를 쓴다
- [ ] 앱 아이콘이 제공된 WHENWORK 아이콘(`assets/icon/whenwork.png`)으로 바뀌고, 트레이용 단순 글리프가 그것에서 파생된다
- [ ] 단축키 등록 실패를 사용자에게 알리고 설정에서 재바인딩할 수 있다. 로그인 시 자동 실행 토글, 알림 실패 시 앱 안 표시 폴백, 트레이·숫자키 첫 실행 안내가 있다
- [x] 공개 리포는 공개 전 비밀 스캔을 통과한다 (새 히스토리 대신 기존 리포를 공개 — 스캔에서 실제 비밀이 없었다)
...
| 앱 이름은 WHENWORK 유지 | WHENNOTE로 바꾸려던 계획은 2026-09-15 철회 — 그 이름은 다른 프로젝트에 쓴다. 공개판도 같은 이름으로 새 리포에서 출발한다 | 2026-09-15 Decided |
| 포그라운드 창 컨텍스트(context.fg) 완전 삭제 | macOS는 화면 녹화 권한이 필요해 미서명 앱과 충돌. 부수 기능에 비해 대가가 크다 | — Pending |
| 공개판은 기존 리포 `when630/whenwork`를 그대로 공개 | 새 히스토리로 시작하려 했으나, 전 히스토리 비밀 스캔(Apps Script 배포 ID·gh/glab/Anthropic 토큰 패턴)에서 실제 값이 나오지 않아 재출발의 이유가 사라졌다. 테스트 자리표시자(`AKfyc...`, `secret-value`)만 남아 있다 | 2026-09-15 Decided |
| 내보내기는 JSON 왕복만 | CSV는 v2. 이전 스크립트도 같은 JSON 형식을 쓴다 | — Pending |
| 앱 아이콘은 사용자가 제공한 WHENWORK 이미지(`assets/icon/whenwork.png`) | 기존 16px 픽셀아트는 개인용 표식이었다. 공개 앱은 설치·Dock에서 알아볼 아이콘이 필요하다 | — Pending |
