...
- [ ] 앱 이름이 WHENNOTE로 바뀐다 (패키지명·productName·appId·설치 파일명·트레이/창 제목·데이터 폴더·문서)
- [ ] 앱 아이콘이 제공된 WHENNOTE 아이콘(`assets/icon/whennote.png`)으로 바뀌고, 트레이용 단순 글리프가 그것에서 파생된다
...
| 앱 이름을 WHENNOTE로 변경 | 공개판은 새 리포에서 새 이름으로 출발한다. "work"보다 "note"가 비개발자 대상 퀵캡처에 맞는다 | — Pending |
...
| 앱 아이콘은 사용자가 제공한 WHENNOTE 이미지 | 기존 16px 픽셀아트는 개인용 표식이었다. 공개 앱은 설치·Dock에서 알아볼 아이콘이 필요하다 | — Pending |
