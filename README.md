...
던진 글은 **그대로** 인박스에 들어갑니다. `#`도 그냥 글자라 "#201 이슈 확인"이 잘리지 않습니다. 분류는 던진 다음에 인박스에서 `1`~`9`로 합니다.
...
**Windows** — 새 버전을 알아서 내려받고, **앱을 종료할 때** 설치합니다. 쓰는 도중에 스스로 재시작하지 않습니다.

**macOS** — 새 버전이 있다고 알려 주되 **자동 설치는 되지 않습니다.** Apple이 서명되지 않은 앱의 자동 업데이트를 막기 때문입니다. 알림을 누르면 받는 곳이 열리니 새 dmg를 내려받아 덮어써 주세요. 데이터는 그대로 유지됩니다.

설정 탭에서 `R`로 언제든 직접 확인할 수 있습니다.
