...
> 각 항목은 [04_로드맵.md](04_로드맵.md)의 단계에 매핑된다. 체크 상태는 2026-09-15 기준.
...
- [x] **DATA-01**: 사용자는 설정에서 전체 데이터(프로젝트·항목·대기·완료 이력)를 사람이 읽을 수 있는 JSON 파일 하나로 내보낼 수 있다
- [x] **DATA-02**: 사용자는 내보낸 JSON을 다른 PC의 앱에서 가져와 같은 상태를 복원할 수 있다. 내보내기→가져오기 왕복 테스트가 있다
- [x] **DATA-03**: 가져오기 직전 현재 데이터가 자동으로 백업되어 잘못 가져와도 되돌릴 수 있다
- [x] **DATA-04**: 기존 PostgreSQL 데이터(project·item·event)를 새 저장소로 한 번 옮기는 스크립트가 있다. 행 수 비교, 타임존(UTC) 보존, context JSON 왕복, soft-delete 필터 판정을 통과한다. 공개 앱 UI에는 노출되지 않는다(작성자 전용)

### 제거 (RMV)

- [x] **RMV-01**: `claude -p` 의존과 재개 카드·인박스 AI 분류·완료 제안·주간 리뷰가 UI·preload·IPC·모듈·설정·저장소·테스트 모든 계층에서 제거된다
- [x] **RMV-02**: git·gh·glab 수집기, 이슈 탭, 리포 상태 추적이 제거된다
- [x] **RMV-03**: Apps Script 캘린더 연동과 오늘 뷰의 일정 타임라인이 제거된다
- [x] **RMV-04**: Obsidian 볼트 출력과 SQL 덤프 백업이 제거된다(백업은 JSON 내보내기가 대신한다)
- [x] **RMV-05**: 캡처 시점 포그라운드 창 제목 추적(context.fg, PowerShell 워커)이 완전히 삭제된다
- [x] **RMV-06**: 제거된 모듈명·IPC 채널명·설정 키가 코드베이스 전체에서 grep 0건이고, 남은 단위 테스트와 스모크가 통과한다
- [x] **RMV-07**: 화면 문구·기본값·테스트 픽스처·문서에서 개인 흔적(특정 프로젝트 약어, 볼트 경로, DB 계정, 캘린더 URL)이 제거된다

### 첫 실행·플랫폼 (PLAT)

- [x] **PLAT-01**: 설치 후 첫 실행에 아무 설정 화면 없이 단축키 캡처가 바로 동작한다
- [x] **PLAT-02**: 단축키 등록이 실패하면 사용자에게 알리고, 설정에서 다른 조합으로 바꿀 수 있다(재바인딩). 바꾼 조합도 등록 성공 여부를 확인한다
- [x] **PLAT-03** *(macOS 실기기 미검증)*: 설정에 로그인 시 자동 실행 토글이 있고, Windows와 미서명 macOS 빌드 실기기에서 실제로 동작함을 확인한다
- [x] **PLAT-04**: 아침 브리핑 알림이 권한 거부나 실패로 표시되지 않으면 앱 안 표시로 대체되어 조용히 사라지지 않는다
- [x] **PLAT-05**: 첫 실행에 트레이 아이콘 위치(OS별 문구)와 인박스 숫자키 분류를 한 번 안내한다
- [x] **PLAT-06** *(macOS 실기기 미검증)*: macOS에서 메뉴바 상주(독 아이콘 없음), 글로벌 단축키, 창 위치 기억, 알림이 동작한다. OS 분기는 플랫폼 모듈 안에 있고 호출부에 흩어지지 않는다

### 이름 (NAME)

- [x] **NAME-01**: 앱 이름은 WHENWORK를 유지한다(2026-09-15 개명 철회). 패키지명, productName, appId, 설치 파일명, 트레이 툴팁, 창 제목, README와 문서가 일관되게 WHENWORK를 쓰고 다른 이름 언급이 남지 않는다
- [x] **NAME-02**: 앱 데이터 폴더는 기존 `whenwork` 그대로 쓰고, 기존 설정 파일(단축키·창 위치·브리핑 시각)은 새 버전 첫 실행에서 그대로 이어진다
- [x] **NAME-03**: 앱 아이콘이 제공된 WHENWORK 아이콘(`assets/icon/whenwork.png`, 1254×1254 RGBA — 파란 그라데이션 원형 시계에 체크 마크, 투명 배경)으로 바뀐다. 설치 파일·실행 파일·창·macOS Dock/dmg에 적용되고, `tools/make-icon.mjs`의 픽셀아트 생성이 이를 덮어쓰지 않는다. 트레이 아이콘은 같은 아이콘에서 파생한 단순 글리프(16·32px, macOS는 템플릿 이미지)로 작게 봐도 알아볼 수 있다
...
| RMV-06 | Phase 2 | Complete |
| RMV-07 | Phase 2 | Complete |
| PLAT-01 | Phase 4 | Complete |
| PLAT-02 | Phase 4 | Complete |
| PLAT-03 | Phase 4 | Complete (macOS 실기기 미검증) |
| PLAT-04 | Phase 4 | Complete |
| PLAT-05 | Phase 4 | Complete |
| PLAT-06 | Phase 4 | Complete (macOS 실기기 미검증) |
...
---
*정의 2026-09-14. 2026-09-15 상태 갱신 후 `.planning/`에서 `docs/`로 이관.*
