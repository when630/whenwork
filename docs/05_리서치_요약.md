...
> 2026-09-14 로드맵 작성 전에 한 리서치(스택·기능·아키텍처·함정 네 갈래)의 요약. 여기서 제안한 6단계는 최종 로드맵([04_로드맵.md](04_로드맵.md))에서 5단계로 합쳐졌다 — 구조 분할과 저장소 교체가 Phase 1로, 삭제가 Phase 2로, 렌더러 정리와 릴리스가 Phase 5로. 핵심 결정(node:sqlite 채택, 큐 크래시 안전성을 완료 게이트로, 삭제 시 grep-to-zero)은 그대로 실행됐다.
...
- Existing codebase, read directly -- main/queue.mjs, main/db.mjs, main/index.mjs, main/context.mjs, main/place.mjs, package.json
...
---
*리서치 2026-09-14. 원본 4편(STACK·FEATURES·ARCHITECTURE·PITFALLS)은 이 요약에 반영된 뒤 정리했다.*
